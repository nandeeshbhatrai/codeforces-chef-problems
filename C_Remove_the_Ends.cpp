...
class Solution {
    public:
        void solve(int t) {
            ...
            ai(a, n);
            // cout << a << '\n';
            vl sumpos(n+1, 0), sumneg(n+1, 0);
            f1r(i , 1,  n+1){
                if(a[i-1] < 0){
                    // cout << ".";
                    sumneg[i] = sumneg[i-1] + abs(a[i-1]);
                    sumpos[i] = sumpos[i-1];
                }else{
                    // cout << "*";
                    sumpos[i] = sumpos[i-1] + a[i-1];
                    sumneg[i] = sumneg[i-1];
                }
            }
            // cout << sumneg << '\n';
            ...
            ans = 0;
            f0r(i , n){
                ans = max(sumpos[i+1] + sumneg.back() - sumneg[i], ans);
            }
            // cout << "ans = ";
            cout << ans << '\n';
        }
