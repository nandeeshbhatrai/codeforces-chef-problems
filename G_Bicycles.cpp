...
void dfs(int u, int end, vector<int>& slowness, vector<vector<int>>& visited, int time, int bike, int& minTime) {
    if (u == end) {
    ...
    for (const Edge& edge : graph[u]) {
        int v = edge.to;
        int w = edge.weight;
        int s = slowness[edge.bike];

        if (bike + 1 <= 100 && !visited[v][bike]) {
            dfs(v, end, slowness, visited, time + w * s, bike + 1, minTime);
        }
    }
...
void solve(int tc = 0){
    ...
    vector<vector<int>> visited(n, vector<int>(101, false));
    int minTime = INF;

    dfs(0, n - 1, slowness, visited, 0, 0, minTime);
