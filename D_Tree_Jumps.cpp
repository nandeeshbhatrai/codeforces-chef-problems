...
public:
    void solve(int t) {
        ...
        // cout << ans << '\n';
        vl tot(depth + 2, 0);
        tot[depth] = ans[depth] % mod;
        f1rd(i, depth - 1, 1) {
            ll curr = ans[i] % mod;//cur me pohochne ka tarika
            ll baap = (mod + ans[i] - 1) % mod;// baap me pohochne ke tarike
            tot[i] = (curr + (baap * tot[i+1]) % mod) % mod;
        }
        ll res = (1 + tot[1]) % mod;
        cout << res << '\n';
    }
