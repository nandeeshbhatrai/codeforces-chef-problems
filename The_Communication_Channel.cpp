...
int bfs(vector<vi>& neighbours , int current_node , int y , vi &vis , vi &memo){
    if(memo[current_node]) {return memo[current_node];}
    if(current_node == y){
        return 0;
    }
    int mn = abs(y - current_node);
    for(auto i: neighbours[current_node]){
        if(!vis[i]){
            vis[i] = 1;
            mn = min(mn , 1 + bfs(neighbours , i , y , vis , memo));
        }
    }
    memo[current_node] = mn;
    return mn;
}
...
void solve(int tc = 0){
    ...
    cin >> x >> y;
    vector<vi> neighbours(n+1);
    for(auto i:a){
        neighbours[i.f].pb(i.s);
        neighbours[i.s].pb(i.f);
    }
    f1r(i , 1 , n+1){
        if(i+1 <= n){
            neighbours[i].pb(i+1);
    ...
    cout << bfs(neighbours, x , y , vis , memo);
}
