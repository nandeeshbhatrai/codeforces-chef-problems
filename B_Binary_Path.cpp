...
void rec(vector<string> &grid, vector<string> &list , int i , int j , string &ans){
    // cout  << "Hello" << '\n';
    //base
    ...
    if(j < grid[0].size()-1 && i != 1 && grid[i+1][j] < grid[i][j+1]){
        ans.push_back(grid[i+1][j]);
        rec(grid , list , i+1 , j , ans);
        ans.pop_back();
    }
    else if(j < grid[0].size()-1 && i != 1 && grid[i+1][j] > grid[i][j+1]){
        ans.push_back(grid[i][j+1]);
        rec(grid , list , i , j+1 , ans);
        ans.pop_back();
    }
    else if(i == 1 && j < grid[0].size()){
        ans.push_back(grid[i][j+1]);
        rec(grid , list , i , j+1, ans);
        ans.pop_back();
    }
