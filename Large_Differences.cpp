...
void solve(int tc = 0){
    ...
    f0r(i , n-1){
        if(i == 0){
            sum = max(abs(k - a[1]) , abs(1 - a[1])) + b.back() - b[0];
            ans = max(ans , sum);
        }else if(i == 1){
            sum = max(abs(a[0] - 1) + abs(a[2] - 1) , abs(a[0] - k) + abs(a[2] - k)) + b.back() - b[i];
            ans = max(ans , sum);
        }else{
            sum = max(abs(a[i-1] - 1) + abs(a[i+1] - 1) , abs(a[i-1] - k) + abs(a[i+1] - k)) + b[i-2] + b.back() - b[i];
            ans = max(ans , sum);
        }
    }
    // last wale ko badlu to
    if(n >= 3){
        sum = max(abs(a[n-2] - 1) , abs(a[n-2] - k)) + b[n-3];
        ans = max(ans , sum);
    }
    cout << ans << '\n';
}
