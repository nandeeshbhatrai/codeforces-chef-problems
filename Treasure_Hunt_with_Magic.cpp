...
class Solution {
    public:
        void solve(int t) {
            ...
            vl a(n);
            ai(a, n);
            vl mem , prev(n, -1) , pos(n, 0);
            f0r(i , n){
                auto it = lower_bound(all(mem) , a[i]);
                ll index = it - mem.begin();
                if(it == mem.end()){
                    mem.pb(a[i]);
                }else{
                    *it = a[i];
                ...
                pos[index] = i;
                prev[i] = index ? pos[index - 1] : -1;
            }
            // cout << mem.size() << '\n';
            vl idx(mem.size());
            for(ll i = pos[mem.size()-1] , j = mem.size()-1; i >= 0; i = prev[i] , --j){
                idx[j] = i;
            }
            // cout << idx << '\n';
            ll skipped = idx[0];
            f0r(i , idx.size()-1){
                ...
                y = a[idx[i+1]];
                // cout << "x = " << x << " y = " << y << '\n';
                // cout << "idx[i+1] = " << idx[i+1] << " idx[i] = " << idx[i] << '\n';
                skipped += min(y-x-1 , idx[i+1] - idx[i] - 1);
            }
            skipped += n - 1 - idx.back();
            // cout << "Skipped = " << skipped << '\n';

            cout << min(skipped , k)+mem.size() << '\n';
        }
    private:
};
