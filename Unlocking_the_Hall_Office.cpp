...
class comp {
public:
    bool operator()(const pair<ll, vl>& a, const pair<ll, vl>& b) {
...
public:
    void solve(int t){
        ...
        priority_queue<pair<ll, vl>, vector<pair<ll, vl>>, comp> pq;
        set<ll> vis;
        pq.push({0, {1}});
        while(!pq.empty()){
            auto [steps, cur] = pq.top();
            pq.pop();
            ll room = cur.back();
