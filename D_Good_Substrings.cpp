...
class Solution {
    public:
        void solve(int t) {
            ...
            cin >> s >> bad >> k;
            n = s.size();
            ll m = 1610612741, p = 31; // 1e9+9 wala submit hua but 1e9+7 wala nahi
            vl h(n+1), p_pow(n), bad_cnt(n+1);
            f0r(i , n){
                bad_cnt[i+1] = bad_cnt[i] + (bad[s[i] - 'a'] == '0');
            }
            p_pow[0] = 1;
            f1r(i, 1, n) {
