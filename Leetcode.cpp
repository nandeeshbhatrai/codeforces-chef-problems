    ...
    ListNode* mergeKLists(vector<ListNode*>& lists) {
        ...
        while(cnt < lists.size()){
            for(int i = 0; i < lists.size(); ++i){
                if(ind == -1 && lists[i] != NULL){
                    ind = i;
                }
                if(lists[i] != NULL && lists[i] -> val <= lists[ind] -> val){
                    ind = i;
                }
            }
            if(head == NULL){
...
int main() {
    ...
    // cin >> tc;
    Solution solution;
    vector<ListNode*> lists = {li({1, 4, 5}), li({1, 3, 4}), li({2, 6})};
    for (int t = 0; t < tc; t++) solution.mergeKLists(lists);
