...
class Solution{
    public:
        void solve(int t , vl& dp){
            ...
            // number hoga ind+1
            // cout << "ind: " << dp[ind-1] << " ind+1 " << dp[ind] << '\n';
            y = k - dp[ind-1];
            ll bits = log10(ind+1)+1;
            // cout << "Bits = " << bits << " y = " << y << '\n';
            ++ind;
            if(y%bits == 0){
                cout << "\t"<<ind%10 << '\n';
            ...
            // mujhe (ind) ka bits-(y%bits)th wala digit(from left) output krna hai
            f0r(i , bits-(y%bits)){
                ind /= 10;
            }
            cout << "\t"<<ind%10 << '\n';
...
int main() {
    ...
    vector<ll> dp(16205749, 0);
    dp[0] = 1;
    f1r(i , 1 , 16205749){
        dp[i] = (i+1)*(floor(log10(i+1)) + 1);
        dp[i] += dp[i-1];
    }
    // f0r(i , 20){
    //     cout << dp[i] << ' ';
